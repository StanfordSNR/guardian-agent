...
/* mark the socket as listening for incoming connections */
void UnixSocket::listen( const int backlog )
{
  SystemCall( "listen", ::listen( fd_num(), backlog ) );
}

/* accept a new incoming connection */
UnixSocket UnixSocket::accept( void )
{
  register_read();
...
void UnixSocket::sendmsg(std::string & payload, const std::vector<int> & fds)
{
	...
	std::string aux_data(CMSG_SPACE(sizeof(int) * fds.size()), '\0'); 
  msg.msg_control = aux_data.data();
	msg.msg_controllen = aux_data.size();

	struct cmsghdr *cmsghdr = CMSG_FIRSTHDR(&msg);
  cmsghdr->cmsg_len = CMSG_LEN(sizeof(int) * fds.size());
  cmsghdr->cmsg_level = SOL_SOCKET;
  cmsghdr->cmsg_type = SCM_RIGHTS;
  int* fd_data = (int*)CMSG_DATA(cmsghdr) ;
  ...
  for (const auto& fd : fds) 
  {
    ...
    *fd_data = fd;
	...
	SystemCall("sendmsg", ::sendmsg(fd_num(), &msg, 0));
}

std::string UnixSocket::recvmsg(std::vector<int>* fds) 
{
	struct msghdr msg;
  msg.msg_name = NULL;
	msg.msg_namelen = 0;
  ...
  msg.msg_iov = iov;
	...
	std::string aux_data(CMSG_SPACE(sizeof(int)), '\0');
	msg.msg_control = aux_data.data();
	msg.msg_controllen = aux_data.size();
  ...
  struct cmsghdr *cmsghdr = CMSG_FIRSTHDR(&msg);
	...
	size_t nbytes = SystemCall("recvmsg", ::recvmsg(fd_num(), &msg, 0));
	fds->push_back(*(int *)CMSG_DATA(cmsghdr));
