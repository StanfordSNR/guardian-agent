...
Address Address::NewUnixAddress(const std::string & path) 
{
  Address address;
  address.addr_.as_sockaddr_un.sun_family = AF_UNIX;
  strcpy(address.addr_.as_sockaddr_un.sun_path, path.c_str());
  address.size_ = sizeof(struct sockaddr_un);
  return address;
}
